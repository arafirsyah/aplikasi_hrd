...
if($nama == '' OR $nik == '' OR $noktp =='' OR $alamat == '' OR $uker == '' OR $no_anggota == '' OR $copy_ktp == '' OR $jml_setoran == '')
{
	...
	mysql_query("UPDATE calonanggota SET nama = '$nama', 
									  nik = '$nik',
									  no_ktp = '$noktp',
									  alamat = '$alamat',
									  uker = '$uker'
									  WHERE nik = '$_POST[nik]'");
	mysql_query("INSERT INTO detail_canggota ('nik','no_anggota','copy_ktp','jml_setoran','tgl_setoran') 
				VALUES ('$nik', '$no_anggota', '$copy_ktp', '$jml_setoran', '$tgl_setoran')");

	echo "<script>alert('Nama : ".$nama." Telah Berhasil diedit')</script>";
		
	/*Setelah proses update selesai data di kembalikan ke form input*/
	//header('location:admin.php');
	echo "<script>window.location='view_canggota1.php'</script>";
}
?>